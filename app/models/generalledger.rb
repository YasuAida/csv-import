class Generalledger < ActiveRecord::Base
  ...
  validates :user_id, uniqueness: { scope: [:pladmin_id, :stock_id, :stockreturn_id, :return_good_id, :disposal_id, :expenseledger_id, :voucher_id, :subexpense_id, :expense_relation_id, :debit_account, :debit_subaccount, :debit_taxcode, :credit_account, :credit_subaccount, :credit_taxcode] }  
  ...
  def self.to_download
    headers = %w(ID user_id pladmin_id stock_id stockreturn_id return_good_id disposal_id expenseledger_id voucher_id subexpense_id expense_relation_id 日付 借方勘定科目 借方補助科目 借方税コード 貸方勘定科目 貸方補助科目 貸方税コード 金額 摘要 購入先)
    csv_data = CSV.generate(headers: headers, write_headers: true, force_quotes: true) do |csv|
      all.each do |row|
        csv_column_values = [
        row.id,
