module GeneralledgersHelper
  ...
  def import_from_pladmins(journalpatterns)
    @pladmins = current_user.pladmins.all
    
    #「損益管理シート」売上計上  
    @pladmins.each do |pladmin|
      find_pattern = journalpatterns.find_by(ledger: "損益管理表",pattern: "売上")
      g_ledger = current_user.generalledgers.build(date: pladmin.date)
    ...
    #「損益管理シート」原価（手数料）計上  
     ...
     #「損益管理シート」入金計上
      ...
      g_ledger = current_user.generalledgers.build(date: pladmin.money_receive)
      g_ledger.pladmin_id = pladmin.id
      g_ledger.debit_account = find_pattern.debit_account
    ...
    #「損益管理シート」手数料支払計上 
      if pladmin.commission_pay_date != nil && pladmin.commission_pay_date != 0
        ...
        g_ledger = current_user.generalledgers.build(date: pladmin.commission_pay_date)
        g_ledger.pladmin_id = pladmin.id
        g_ledger.debit_account = find_pattern.debit_account
    ...
    #「損益管理シート」マルチチャンネル発送分売掛金相殺および送料支払計上    
      if pladmin.shipping_pay_date != nil && pladmin.shipping_pay_date != 0
        if pladmin.goods_name.include?("マルチ発送分")    
          ...
          g_ledger = current_user.generalledgers.build(date: pladmin.shipping_pay_date)
          g_ledger.pladmin_id = pladmin.id
          g_ledger.debit_account = find_pattern.debit_account
  ...
  def import_from_stocks(journalpatterns)
    ...
    @stocks.each do |stock|
      ...
      g_ledger = current_user.generalledgers.build(date: stock.date)
      ...
      if g_ledger.save
        ...
        old_data = current_user.generalledgers.find_by(stock_id: stock.id, date: g_ledger.date, debit_account: g_ledger.debit_account, debit_subaccount: g_ledger.debit_subaccount, debit_taxcode: g_ledger.debit_taxcode, credit_account: g_ledger.credit_account, credit_subaccount: g_ledger.credit_subaccount, credit_taxcode: g_ledger.credit_taxcode )
        old_data.update(date: g_ledger.date, amount: g_ledger.amount, content: g_ledger.content, trade_company: g_ledger.trade_company) if old_data
      end
    ...
    #「仕入台帳」掛払い支払計上     
      if stock.date != stock.money_paid
        ...
        g_ledger = current_user.generalledgers.build(date: stock.money_paid)
        g_ledger.stock_id = stock.id
        g_ledger.debit_account = find_pattern.debit_account
        ...
        if g_ledger.save
          ...
          old_data = current_user.generalledgers.find_by(stock_id: stock.id, debit_account: g_ledger.debit_account, debit_subaccount: g_ledger.debit_subaccount, debit_taxcode: g_ledger.debit_taxcode, credit_account: g_ledger.credit_account, credit_subaccount: g_ledger.credit_subaccount, credit_taxcode: g_ledger.credit_taxcode )
          old_data.update(date: g_ledger.date, amount: g_ledger.amount, content: g_ledger.content, trade_company: g_ledger.trade_company) if old_data
        end
      end
    end
  ...
  def import_from_stockreturns(journalpatterns)
    #「仕入返品台帳」返品計上
    @stockreturns = current_user.stockreturns.all
    @stockreturns.each do |stockreturn|
      ...
      g_ledger = current_user.generalledgers.build(date: stockreturn.date)
      ...
      if g_ledger.save
        ...
        old_data = current_user.generalledgers.find_by(stockreturn_id: stockreturn.id, debit_account: g_ledger.debit_account, debit_subaccount: g_ledger.debit_subaccount, debit_taxcode: g_ledger.debit_taxcode, credit_account: g_ledger.credit_account, credit_subaccount: g_ledger.credit_subaccount, credit_taxcode: g_ledger.credit_taxcode )
        old_data.update(date: g_ledger.date, amount: g_ledger.amount, content: g_ledger.content, trade_company: g_ledger.trade_company) if old_data
      end
    ...
    #「仕入返品台帳」払い戻し受領計上     
      if stockreturn.date != stockreturn.money_paid
        ...
        g_ledger = current_user.generalledgers.build(date: stockreturn.money_paid)
        g_ledger.stockreturn_id = stockreturn.id        
        g_ledger.debit_account = find_pattern.debit_account
        g_ledger.debit_subaccount = ""
        g_ledger.debit_taxcode = find_pattern.debit_taxcode
  ...
  def import_from_stockledgers(journalpatterns)
    ...
    @return_goods.each do |return_good|
      ...
      g_ledger.credit_taxcode = find_pattern.credit_taxcode
      return_stock = current_user.stocks.find(return_good.stock_id)
      g_ledger.content = return_stock.goods_name
      g_ledger.trade_company = return_stock.purchase_from
      return_stockledger = current_user.stockledgers.find_by(stock_id: return_good.stock_id, classification: "返還")
      g_ledger.amount = return_stockledger.grandtotal * -1
      if g_ledger.save
      else
        old_data = current_user.generalledgers.find_by(return_good_id: return_good.id, debit_account: g_ledger.debit_account, debit_subaccount: g_ledger.debit_subaccount, debit_taxcode: g_ledger.debit_taxcode, credit_account: g_ledger.credit_account, credit_subaccount: g_ledger.credit_subaccount, credit_taxcode: g_ledger.credit_taxcode )
        old_data.update(date: g_ledger.date, amount: g_ledger.amount, content: g_ledger.content, trade_company: g_ledger.trade_company) if old_data
      end
    end
    
    #「商品有高帳」商品を廃棄した場合    
    @disposal_goods = current_user.disposals.all
    @disposal_goods.each do |disposal_good|
      if current_user.stockledgers.find_by(sku: disposal_good.sku, classification: "廃棄").present?
        find_pattern = journalpatterns.find_by(ledger: "商品有高帳",pattern: "廃棄")
        @disposal_stockledger = current_user.stockledgers.find_by(sku: disposal_good.sku, classification: "廃棄")
        g_ledger = current_user.generalledgers.build(date: @disposal_stockledger.transaction_date)
        ...
        if current_user.stocks.find_by(sku: disposal_good.sku).present?
          stock_trade_company = current_user.stocks.find_by(sku: disposal_good.sku)
        elsif current_user.return_goods.find_by(new_sku: disposal_good.sku).present?
          return_good = current_user.return_goods.find_by(new_sku: disposal_good.sku)
          stock_trade_company = current_user.stocks.find_by(sku: return_good.old_sku)
        end
        ...
        if g_ledger.save
          ...
          old_data = current_user.generalledgers.find_by(disposal_id: disposal_good.id, debit_account: g_ledger.debit_account, debit_subaccount: g_ledger.debit_subaccount, debit_taxcode: g_ledger.debit_taxcode, credit_account: g_ledger.credit_account, credit_subaccount: g_ledger.credit_subaccount, credit_taxcode: g_ledger.credit_taxcode )
          old_data.update(date: g_ledger.date, amount: g_ledger.amount, content: g_ledger.content, trade_company: g_ledger.trade_company) if old_data
        end
      end
    end    
  ...
  def import_from_subexpenses(journalpatterns)
    ...
    @subexpenses.each do |subexpense|
      ...
      g_ledger = current_user.generalledgers.build(date: subexpense.date)
      ...
      targetstock = current_user.stocks.find(targetgoods[0].to_i)
      ...
      if g_ledger.save
        ...
        old_data = current_user.generalledgers.find_by(subexpense_id: subexpense.id, debit_account: g_ledger.debit_account, debit_subaccount: g_ledger.debit_subaccount, debit_taxcode: g_ledger.debit_taxcode, credit_account: g_ledger.credit_account, credit_subaccount: g_ledger.credit_subaccount, credit_taxcode: g_ledger.credit_taxcode )
        old_data.update(date: g_ledger.date, amount: g_ledger.amount, content: g_ledger.content, trade_company: g_ledger.trade_company) if old_data
      end
    ...
    #「付随費用」掛払い支払計上
      if subexpense.date != subexpense.money_paid
        ...
        g_ledger = current_user.generalledgers.build(date: subexpense.money_paid)
        g_ledger.subexpense_id = subexpense.id          
        g_ledger.debit_account = find_pattern.debit_account
    ...
    @expense_relations.each do |expense_relation|
      @target_subexpense = current_user.subexpenses.find(expense_relation.subexpense_id)
      @target_stock = current_user.stocks.find(expense_relation.stock_id)      
      ...
      g_ledger = current_user.generalledgers.build(date: @target_subexpense.date)
      ...
      if g_ledger.save
        ...
        old_data = current_user.generalledgers.find_by(expense_relation_id: expense_relation.id, debit_account: g_ledger.debit_account, debit_subaccount: g_ledger.debit_subaccount, debit_taxcode: g_ledger.debit_taxcode, credit_account: g_ledger.credit_account, credit_subaccount: g_ledger.credit_subaccount, credit_taxcode: g_ledger.credit_taxcode )
        old_data.update(date: g_ledger.date, amount: g_ledger.amount, content: g_ledger.content, trade_company: g_ledger.trade_company) if old_data
      end
    end
  end        
  ...
  def import_from_expenseledgers(journalpatterns)
    #「経費帳」経費発生による計上
    @expenseledgers = current_user.expenseledgers.all
    @expenseledgers.each do |expenseledger|
      if current_user.accounts.where(account: expenseledger.account_name).blank?
         current_user.accounts.create(account: expenseledger.account_name)
      end      
      
      if expenseledger.purchase_from == "Amazon"
        find_pattern = journalpatterns.find_by(ledger: "経費帳", pattern: "Amazon経費")   
        g_ledger = current_user.generalledgers.build(date: expenseledger.date)
        ...
        if g_ledger.save
          ...
          old_data = current_user.generalledgers.find_by(expenseledger_id: expenseledger.id, debit_account: g_ledger.debit_account, debit_subaccount: g_ledger.debit_subaccount, debit_taxcode: g_ledger.debit_taxcode, credit_account: g_ledger.credit_account, credit_subaccount: g_ledger.credit_subaccount, credit_taxcode: g_ledger.credit_taxcode )
          old_data.update(date: g_ledger.date, amount: g_ledger.amount, content: g_ledger.content, trade_company: g_ledger.trade_company) if old_data
        end        
      else
        #仕訳パターンを選ぶ
    ...
    #「経費帳」Amazon経費売掛金相殺および掛払い支払計上
      if expenseledger.purchase_from == "Amazon"
        ...
        g_ledger = current_user.generalledgers.build(date: expenseledger.money_paid) 
        g_ledger.expenseledger_id = expenseledger.id
        g_ledger.debit_account = find_pattern.debit_account
  ...
  def import_from_vouchers
    ...
    @vouchers.each do |voucher|
      g_ledger = current_user.generalledgers.build(date: voucher.date)
      ...
      if g_ledger.save
        ...
        old_data = current_user.generalledgers.find_by(voucher_id: voucher.id, debit_account: g_ledger.debit_account, debit_subaccount: g_ledger.debit_subaccount, debit_taxcode: g_ledger.debit_taxcode, credit_account: g_ledger.credit_account, credit_subaccount: g_ledger.credit_subaccount, credit_taxcode: g_ledger.credit_taxcode )
        old_data.update(date: g_ledger.date, amount: g_ledger.amount, content: g_ledger.content, trade_company: g_ledger.trade_company) if old_data
      end     
    end    
  end
