  ...
  def import_to_pladmin
    current_user.sales.all.each do |sale|
      if sale.handling == "原価（手数料）" || sale.handling.blank?
        ...
        current_user.pladmins.create(sale_id: sale.id, date: sale.date, order_num: sale.order_num, sku: sale.sku, goods_name: sale.goods_name, quantity: sale.quantity, sale_place: "Amazon", sale_amount: sale.amount, commission: commission*-1.to_i, money_receive: sale.money_receive)
        ...
        if sale.goods_name.present?
          ...
          current_user.pladmins.create(sale_id: sale.id, date: sale.date, order_num: sale.order_num, sku: sale.sku, goods_name: name_of_goods, quantity: 1, sale_place: "Amazon", sale_amount: sale.amount, commission: nil, money_receive: sale.money_receive)  
      
      elsif sale.handling == "原価（送料）"
        shipping_cost = current_user.sales.where(date: sale.date, order_num: sale.order_num).sum(:amount) 
        double_pladmins = current_user.pladmins.where(date: sale.date, order_num: sale.order_num, quantity: 1, sale_place: "その他", shipping_cost: shipping_cost * -1, shipping_pay_date: sale.money_receive)
        if double_pladmins.blank?
          pladmin = current_user.pladmins.build(sale_id: sale.id, date: sale.date, order_num: sale.order_num, quantity: 1, sale_place: "その他", shipping_cost: shipping_cost * -1, shipping_pay_date: sale.money_receive)
          pladmin.save
        ...
        expenseledger = current_user.expenseledgers.build(sale_id: sale.id, date: sale.date,account_name: "支払手数料", content: sale.detail_of_payment, amount: (sale.amount * -1), money_paid: sale.money_receive, purchase_from: "Amazon", currency: "円")
