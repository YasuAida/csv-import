module PladminsHelper
  ...
  def merge_multi_channel
    @ex_merge_pladmins = current_user.pladmins.where.not(sale_place: "Amazon")
    @merge_pladmins = @ex_merge_pladmins.where.not(sale_place: "その他")
    @merge_pladmins.each do |merge_pladmin|
      @target_pladmins = current_user.pladmins.where(sale_place: "その他", sku: merge_pladmin.sku).order(:date) 
